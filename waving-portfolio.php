...
define("WAVING_PORTFOLIO_VERSION", "1.2.2");
  ...
  function load_frontend_libraries() {
    ...
    // Loading waving hover effect scripts
      wp_enqueue_script(
          ...
          array( 'jquery' ), WAVING_PORTFOLIO_VERSION
      );
          wp_enqueue_script(
          'waving-portfolio-modernizer-script',
          plugins_url( 'assets/js/modernizr.custom.97074.js' , __FILE__ ), '', WAVING_PORTFOLIO_VERSION
      );
          ...
          wp_register_script('waving-portfolio-custom-script', 
                  plugins_url( 'assets/js/custom.js' , __FILE__ ),'',WAVING_PORTFOLIO_VERSION, true);
  ...
  */
  ...
  function Building_Portfolio_List($width, $height, $fx, $theme, $tag, $showCategory,$all, $click)
  {
      ...
      if(count($cat_lists)!=0){
        ...
        {
          $categoryMenu[] = '<button class="waving-button action-button shadow animate blue" onClick="ShowLists(\''.$term_cat->slug.'\',\''.$tagCondition.'\')">'.$term_cat->name.'</button>';
        }
      }
